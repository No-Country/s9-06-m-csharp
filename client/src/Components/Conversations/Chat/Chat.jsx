...
const Chat = () => {
	...
	const handleSendMessage = e => {
		e.preventDefault()
		if (message.trim() !== '') {
			socket.emit('send-message', { fromUserId: 'user 1', message }) // Cambia 'user 1' por el ID del usuario actual
			setMessage('')
		}
	}

	//obteniedo mensaje del socket.io
	useEffect(() => {
		socket.current.on('recieve-message', data => {
			console.log(data)
			setChatLog(data) //este es de la conversa o no??
		})
	}, [])
